...
const InviteUserToTripForm: React.FC<InviteUserToTripFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/trips/${trip.id}/invites`,
        {
          method: 'POST',
          headers: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      ></div>
      <div className="relative w-full max-w-md mx-auto p-5 bg-white rounded shadow-md z-10">
        <h2 className="text-2xl font-bold mb-4">
          Invite User to {trip.name} Gallery
        </h2>
